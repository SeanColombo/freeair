 ...
 * widgets" inside the app. Never triggers a network fetch -- a widget with no API key saved yet
 * (app-global) or no sensor ID (per-widget) shows as [FreeAirWidgetState.NeedsSetup]; a fully
 * configured one shows its last cached reading, or the last cached failure (e.g. an invalid
 * sensor ID) if it never had a successful reading, or [FreeAirWidgetState.Loading] if neither
 * exists yet (just set up, hasn't fetched once). The reading/error cache is shared with the
 * widget's own render path (see [FreeAirWidget]), so whichever one fetches first updates what
 * the other shows here too.
 */
suspend fun loadWidgetPreviews(context: Context): List<WidgetPreviewItem> {
    ...
    val hasApiKey = loadApiKey(context) != null
    return glanceIds.map { id ->
        ...
        val state = if (sensorConfig == null || !hasApiKey) {
